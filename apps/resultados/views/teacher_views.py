...
def filtrar_resultados(filtros):
    """
    Filtra resultados por campos exactos relacionados a la inscripción y convocatoria.
    """
    ...
    if facultad:
        resultados = resultados.filter(inscripcion__estudiante__facultad__icontains=facultad)
    if grupo:
        resultados = resultados.filter(inscripcion__estudiante__grupo__icontains=grupo)
    if anio_escolar:
...
def buscar_en_resultados(resultados, buscar):
    ...
    if buscar:
        return resultados.filter(
            ...
            Q(inscripcion__fecha_inscripcion__icontains=buscar) |
            Q(nota__icontains=buscar) |
            Q(notaL__icontains=buscar) |
            Q(notaE__icontains=buscar) |
            Q(notaC__icontains=buscar) |
            Q(notaO__icontains=buscar)
        )
    return resultados
